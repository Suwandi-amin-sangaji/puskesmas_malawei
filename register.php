...
if (isset($_POST['register'])) {
    ...
    $verifikasi_password = md5($_POST['verifikasi_password']);

    // Cek apakah email sudah terdaftar
    $cek_email = mysqli_query($koneksi, "SELECT * FROM tb_users WHERE email ='$email'");
    if (mysqli_num_rows($cek_email) > 0) {
        echo "<script> alert('email sudah terdaftar. Silakan gunakan email lain.')</script>";
    } else {
        // Verifikasi password
        if ($password != $verifikasi_password) {
            echo "<script> alert('Password tidak sesuai dengan verifikasi password.')</script>";
        } else {
            // Proses registrasi pasien
            $nama = $_POST['nama'];
            $level = 'pasien';

            $query = mysqli_query($koneksi, "INSERT INTO tb_users (email, password, nama, username, level) VALUES ('$email', '$password', '$nama', '-','$level')");
            if ($query) {
                $_SESSION['email'] = $email; // Simpan username dalam session
                $_SESSION['nama'] = $nama; // Simpan nama dalam session
...
<body>
  ...
  <div class="login-container">
    ...
    <div class="form__field form__field--email">
        ...
        <div class="form__input-wrapper">
          <input id="login__email" type="text" name="nama" class="form__input" placeholder="Nama Lengkap" required>
      ...
      <div class="form__field form__field--password">
        <label class="label label--icon" for="login__password">
          ...
          <span class="hidden">Verifikasi Password</span>
        </label>
        <div class="form__input-wrapper">
          <input id="login__password" type="password" name="verifikasi_password" class="form__input" placeholder="Verifikasi Password" required>
      ...
      <div class="login-actions-container">
        ...
        <a class="forgot-password" href="login">LOGIN?</a>
  ...
  <script src="assets/cartoon.js"></script>
</body>
